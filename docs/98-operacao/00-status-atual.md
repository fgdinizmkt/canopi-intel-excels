...
- **HubSpot Ingest Contract Foundation (C2.9E.2A)** fechado tecnicamente em `b6c9d4a`: migration `hubspot_ingest_contracts`, tipos `hubspotIngestTypes.ts`, service server-side `hubspotIngestContractService.ts`, rota `/api/account-connectors/hubspot/ingest/contracts`, dry-run summary e representação de Companies + Contacts sem ingestão real. Validação funcional real pendente.
