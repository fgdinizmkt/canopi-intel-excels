...
const SectionHeader: React.FC<{ 
  ...
  <div className="flex justify-between items-start mb-6">
    ...
    {badge && (
      <Badge className={`bg-${badgeColor}-500/10 text-${badgeColor}-600 border-${badgeColor}-200/50 text-[10px] font-black uppercase tracking-widest px-3 py-1`}>
        {badge}
      </Badge>
    )}
  </div>
  ...
  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
    {[
      ...
      <Card key={i} className="p-8 border-slate-200 shadow-sm rounded-[40px] bg-white hover:shadow-lg transition-all border-b-8" style={{ borderBottomColor: rank.color === 'emerald' ? '#10b981' : rank.color === 'blue' ? '#3b82f6' : '#ef4444' }}>
        ...
        <Button variant="ghost" className="w-full mt-6 text-[10px] font-black uppercase tracking-widest text-slate-400 hover:text-blue-600 gap-2">Ver Ranking Completo <ArrowRight className="w-3 h-3" /></Button>
      </Card>
    ))}
  </div>
);
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
      {accounts.slice(0, 4).map((acc, i) => (
        <Card key={i} className="p-8 border-slate-200 shadow-sm rounded-[40px] bg-white hover:shadow-md transition-all">
          ...
          <Button variant="ghost" className="w-full mt-6 text-[9px] font-black uppercase text-slate-400 hover:text-blue-600">Ver Ficha Completa</Button>
        </Card>
      ))}
    </div>
  </div>
          ...
          {accounts.slice(0, 15).map((acc, i) => (
            <tr key={i} className="hover:bg-blue-50/30 transition-all group cursor-pointer" onClick={() => onSelect(acc)}>
              ...
              <td className="px-8 py-6 text-right">
                 <Button size="sm" className="bg-white border-2 border-slate-200 text-slate-400 group-hover:bg-blue-600 group-hover:text-white group-hover:border-blue-600 rounded-xl font-black text-[10px] uppercase px-5 transition-all">
                   Ver 360°
                 </Button>
              </td>
            </tr>
