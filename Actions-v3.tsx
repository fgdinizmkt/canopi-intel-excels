...
function statusSummaryLabel(status: ActionStatus) {
  if (status === "Nova") return "Aguardando início";
  if (status === "Em andamento") return "Execução em curso";
  if (status === "Bloqueada") return "Aguardando desbloqueio";
  if (status === "Adiada") return "Reprogramada";
  return "Concluída";
}
...
function ActionProfileModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center bg-slate-950/35 p-6 backdrop-blur-[2px]">
      <div className="max-h-[92vh] w-full max-w-6xl overflow-hidden rounded-[32px] border border-slate-200 bg-white shadow-2xl">
        <div className="border-b border-slate-200 px-8 py-6">
          <div className="flex items-start justify-between gap-6">
            <div>
              ...
              <h2 className="mt-4 text-3xl font-black tracking-tight text-slate-900">{action.title}</h2>
            ...
            {(["Resumo", "Projeto", "Histórico"] as ModalTab[]).map((item) => (
              <button
                ...
                className={cx(
                  ...
                  tab === item ? "bg-slate-900 text-white" : "border border-slate-200 bg-white text-slate-600 hover:bg-slate-50"
                )}
              >
                {item}
              </button>
        ...
        <div className="max-h-[calc(92vh-184px)] overflow-y-auto px-8 py-6">
          {tab === "Resumo" ? (
            <div className="grid gap-6 lg:grid-cols-[1.4fr_0.9fr]">
              <div className="space-y-6">
                <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
                  ...
                  <InfoCard label="Prazo" value={action.dueLabel} subvalue={action.dueDate} />
                </div>

                <div className="grid gap-4 md:grid-cols-2">
                  <TextCard title="Impacto esperado" content={action.expectedImpact} />
                  <TextCard title="Próximo passo" content={action.nextStep} />
                </div>
                ...
                <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
                  ...
                  <ListCard title="Contatos e envolvidos" items={action.contacts} emptyLabel="Sem contatos relacionados." />
                </div>
              ...
              <div className="space-y-6">
                <div className="rounded-3xl border border-slate-200 bg-slate-50 p-5">
                  ...
                  <div className="mt-4 space-y-3">
                    ...
                    >
                      {action.ownerName ? "Reatribuir owner" : "Atribuir owner sugerido"}
                    </button>
                    <button
                      type="button"
                    ...
                    <button
                      type="button"
                      onClick={() => onStatusChange(action.id, action.status === "Adiada" ? "Nova" : "Adiada")}
                      className="w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm font-semibold text-slate-700 transition hover:bg-slate-100"
                    >
                      {action.status === "Adiada" ? "Reabrir ação" : "Adiar ação"}
                    </button>
                  </div>
                </div>

                <div className="rounded-3xl border border-slate-200 bg-white p-5">
                  <p className="text-[11px] font-semibold uppercase tracking-[0.2em] text-slate-400">Linha do tempo</p>
                  <div className="mt-4 space-y-4">
                    {action.timeline.map((step) => (
                      <div key={step.id} className="flex gap-3">
                        <div className="flex flex-col items-center">
                          <span className={cx("mt-1 h-3 w-3 rounded-full", timelineDot[step.status])} />
                          <span className="mt-1 h-full w-px bg-slate-200" />
                        </div>
                        <div className="pb-3">
                          <p className="text-sm font-semibold text-slate-900">{step.label}</p>
                          <p className="mt-1 text-sm text-slate-600">{step.note}</p>
                          <p className="mt-2 text-xs font-medium text-slate-400">{step.owner} • {step.dueLabel}</p>
                        </div>
                      </div>
                    ))}
                  </div>
            ...
            <div className="space-y-6">
              <div className="rounded-3xl border border-slate-200 bg-white p-6">
                <div className="flex flex-wrap items-center justify-between gap-4">
                  <div>
                    <p className="text-[11px] font-semibold uppercase tracking-[0.2em] text-slate-400">Visualização do processo da ação</p>
                    <h3 className="mt-2 text-xl font-black tracking-tight text-slate-900">Projeto operacional vinculado à ação</h3>
                    <p className="mt-2 text-sm text-slate-600">Aqui o Gantt deixa de ser da página e passa a representar o fluxo desta ação específica.</p>
                  </div>
                  <div className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-600">
                    Status atual: <span className="font-semibold text-slate-900">{statusSummaryLabel(action.status)}</span>
                  </div>
                </div>

                <div className="mt-6 overflow-x-auto rounded-3xl border border-slate-200 bg-slate-950 p-4 text-white">
                  <div className="min-w-[1100px]">
                    <div className="grid grid-cols-[260px_repeat(24,minmax(0,1fr))] border-b border-sky-400/30 text-sm">
                      <div className="border-r border-sky-400/30 p-4 text-left text-lg font-semibold">Projeto da ação</div>
                      {MONTHS.map((month, monthIndex) => (
                        <div key={month} className="col-span-4 border-r border-sky-400/30 p-3 text-center last:border-r-0">
                          <div className="text-3xl font-light">{month}</div>
                          <div className="mt-2 grid grid-cols-4 gap-2 text-xs text-sky-100/80">
                            {[1, 2, 3, 4].map((week) => (
                              <span key={`${month}-${week}`}>W{week}</span>
                            ))}
                          </div>
                        </div>
                      ))}
                    ...
                    {projectLanes.map((lane) => (
                      <div key={lane} className="grid grid-cols-[260px_repeat(24,minmax(0,1fr))] border-b border-sky-400/20 last:border-b-0">
                        <div className="border-r border-sky-400/20 p-4">
                          <p className="text-2xl font-light">{lane}</p>
                          <p className="mt-2 text-sm text-sky-100/70">{action.projectPlan.filter((item) => item.lane === lane).length} etapa(s)</p>
                        </div>
                        <div className="col-span-24 p-4">
                          <div className="space-y-4">
                            {action.projectPlan
                              .filter((item) => item.lane === lane)
                              .map((task) => (
                                <div key={task.id} className="grid grid-cols-[220px_1fr] items-center gap-4">
                                  <div>
                                    <p className="text-sm font-semibold text-white">{task.task}</p>
                                    <p className="text-xs text-sky-100/70">{task.owner}</p>
                                  </div>
                                  <div className="relative grid h-10 grid-cols-24 gap-2">
                                    {Array.from({ length: TOTAL_WEEKS }).map((_, index) => (
                                      <div key={`${task.id}-${index}`} className="rounded-md bg-white/5" />
                                    ))}
                                    <div
                                      className={cx(
                                        "absolute top-1.5 h-7 rounded-full px-3 text-xs font-semibold leading-7 text-white shadow-lg",
                                        laneStyle[task.status]
                                      )}
                                      style={{ left: `${((task.startWeek - 1) / TOTAL_WEEKS) * 100}%`, width: `${(task.duration / TOTAL_WEEKS) * 100}%` }}
                                    >
                                      {task.task}
                                    </div>
                                  </div>
          ...
          {tab === "Histórico" ? (
            <div className="grid gap-6 lg:grid-cols-[1.1fr_0.9fr]">
              <div className="rounded-3xl border border-slate-200 bg-white p-6">
                <p className="text-[11px] font-semibold uppercase tracking-[0.2em] text-slate-400">Histórico de decisões</p>
              ...
              <div className="space-y-6">
                <ListCard title="Tags" items={action.tags} emptyLabel="Sem tags." />
                <ListCard title="Dependências em aberto" items={action.dependencies} emptyLabel="Nenhuma dependência." />
                <ListCard title="Bloqueios" items={action.blockers} emptyLabel="Nenhum bloqueio ativo." />
              </div>
            </div>
          ) : null}
...
export default function ActionsPage() {
  ...
  const [notice, setNotice] = useState<string>("Kanban arrastável ativo. Arraste ações entre colunas para simular operação real.");
  ...
  function handleAssignOwner(id: string) {
    ...
    setNotice(`Owner atribuído: ${target.suggestedOwner} em “${target.title}”.`);
  }
  ...
  function handleEscalate(id: string) {
    ...
    setNotice(`Risco escalado em “${target.title}”. Prioridade ajustada para Crítica.`);
  }
  ...
  function handleStatusChange(id: string, nextStatus: ActionStatus) {
    ...
    setNotice(`Status de “${target.title}” alterado para ${nextStatus}.`);
  }
        ...
        <main className="flex-1">
          <header className="border-b border-slate-200 bg-white">
            <div className="flex flex-wrap items-center justify-between gap-4 px-6 py-5 lg:px-10">
              <div>
                <p className="text-[11px] font-semibold uppercase tracking-[0.24em] text-slate-400">Canopi • Revenue Ops • Fila de ações</p>
                <h1 className="mt-2 text-3xl font-black tracking-tight text-slate-900">Fila operacional de ações</h1>
                <p className="mt-2 text-sm text-slate-500">Ações reais com contexto de conta, sinal, owner, SLA, histórico e processo operacional por ação.</p>
              </div>
          ...
          <div className="px-6 py-6 lg:px-10">
            ...
            <div className="mt-6 rounded-[30px] border border-slate-200 bg-white p-4 shadow-sm">
                    ...
                    {STATUS_ORDER.map((lane) => {
                      ...
                      return (
                        <div
                          ...
                          className="rounded-[30px] border border-slate-200 bg-white p-4 shadow-sm"
                        >
                            ...
                            {laneItems.length ? (
                              laneItems.map((action) => (
                                <div
                                  ...
                                  onClick={() => setSelectedId(action.id)}
                                  className={cx(
                                    "cursor-grab rounded-[26px] border border-slate-200 bg-slate-50 p-4 transition active:cursor-grabbing",
                                    selectedAction?.id === action.id ? "border-blue-300 ring-2 ring-blue-100" : ""
                                  )}
                                >
                                  <div className="flex flex-wrap gap-2">
                                    <span className={cx("rounded-full px-3 py-1 text-[11px] font-bold", priorityPill[action.priority])}>{action.priority}</span>
                                    <span className={cx("rounded-full px-3 py-1 text-[11px] font-bold", slaPill[action.slaStatus])}>{formatSlaLabel(action.slaStatus)}</span>
                                  </div>
                                  <p className="mt-3 text-base font-black tracking-tight text-slate-900">{action.title}</p>
                                  <p className="mt-2 text-sm leading-6 text-slate-600">{action.description}</p>
                                  <div className="mt-4 space-y-1.5 text-sm text-slate-500">
                                    <p><span className="font-semibold text-slate-700">Conta:</span> {action.accountName}</p>
                                    <p><span className="font-semibold text-slate-700">Owner:</span> {action.ownerName ?? "Não atribuído"}</p>
                                    <p><span className="font-semibold text-slate-700">Próximo passo:</span> {action.nextStep}</p>
                                  </div>
                                  <div className="mt-4 flex gap-2">
                                    <button
                                      ...
                                      className="flex-1 rounded-2xl border border-slate-200 bg-white px-3 py-2 text-sm font-semibold text-slate-700 transition hover:bg-slate-100"
                                    >
                                      Ver
                                    </button>
                                    <button
                                      ...
                                      className="flex-1 rounded-2xl bg-blue-600 px-3 py-2 text-sm font-semibold text-white transition hover:bg-blue-700"
                                    >
                                      Avançar
                                    </button>
                                  </div>
                              ...
                              <div className="rounded-[26px] border border-dashed border-slate-200 bg-slate-50 p-5 text-center text-sm text-slate-400">
                                Arraste uma ação para esta coluna.
                              </div>
                            )}
                          </div>
                        </div>
