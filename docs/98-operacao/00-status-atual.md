...
**HubSpot C2.9E.2D.14** fechado: 27 Deals criados limpos no HubSpot, 27 associações Deal → Company e 80 associações Deal → Contact criadas via API v4, 27 mappings `entity_type=deal` persistidos em `hubspot_identity_mappings`, `canProceedToNextRecorte: true`, 0 falhas, 0 Contacts/Companies/Products criados, 0 escrita em `accounts`/`contacts`/`oportunidades` no Supabase, Contacts HubSpot permaneceram em 993; incidente de execução antecipada do create real resolvido via recovery sem create adicional; constraint Supabase ajustada para aceitar `entity_type=deal`; serviço corrigido com `preflightOnly` real; `preflightOnly=true` confirmado (diagnóstico completo, 0 writes); idempotência confirmada (segunda chamada bloqueada com 27 mappings ativos); doc operacional em `66-hubspot-clean-reload-deal-create.md`.
...
**HubSpot C2.9E.2D.12B** fechado como auditoria pós-create de Companies: 665 Companies auditadas, 247 novas operacionais com `canopi_canonical_id`, 348 históricas com `canopi_company_id`, 70 fora do padrão Canopi, 0 duplicidades reais entre nova base e histórico, 89 duplicidades visuais sintéticas explicadas pelo dataset C4.17.1, 247/247 mappings íntegros com `hs_object_id` confirmado e nenhum enriquecimento real ainda; Contacts é o próximo recorte recomendado.

## Branch principal
`main` alinhado com `origin/main`. Commits publicados: `5a3a019` (`feat(settings): add HubSpot read-only connector surface`), `404952c` (documental), `e4b4585` (`fix(settings): persist HubSpot read-only connection state`), `43ed689` (`feat(settings): add HubSpot writeback dry-run flow`), `833b1f2` (documental), `a7b3034` (`feat(settings): add HubSpot writeback setup flow`), `1b5ab47` (documental), `0c0f70b` (`feat(settings): add protected HubSpot writeback execution`), `446da55` (`feat(settings): add HubSpot read-only CRM snapshot`), `b6c9d4a` (`feat(settings): add HubSpot ingest contract foundation`), `93491cf` (`feat(settings): add HubSpot ingest execution dry-run`), `6b19b38` (`feat(settings): add HubSpot ingest execution plan snapshot`), `f00ac0a` (`feat(settings): add HubSpot ingest apply RPC foundation`), `535f8ca` (`feat(settings): add protected HubSpot ingest apply route`) e `3193a5e` (`feat(settings): add HubSpot identity mapping foundation`). **HubSpot read-only connector surface (C2.9B)** está implementado, validado funcionalmente no browser e **fechado operacionalmente** no escopo read-only. **HubSpot C2.9C está fechado como dry-run/preparação**. **HubSpot C2.9D.1 está fechado tecnicamente e publicado**. **HubSpot C2.9D.2 está fechado tecnicamente e validado funcionalmente**, com base completa subida e conferida no HubSpot. **HubSpot read-only CRM snapshot está fechado tecnicamente e validado funcionalmente**, com Contacts total batendo com o HubSpot e diferença residual de 2 contatos sem `canopi_contact_id` para backlog futuro. **HubSpot ingest contract foundation (C2.9E.2A) está validado funcionalmente com contrato real, GET por id confirmando o mesmo contrato e sem retorno de token**. **HubSpot ingest execution dry-run (C2.9E.2B.1) está fechado tecnicamente e validado funcionalmente**, com `canPersist=false`, `summary.persisted=false`, `summary.dryRunOnly=true`, nenhum dado persistido e bloqueio explícito de payload proibido. **HubSpot ingest execution plan snapshot (C2.9E.2B.2A) está fechado tecnicamente e validado funcionalmente**, com snapshot materializado, `planHash` determinístico, `execution_summary` salvo, `contract_json` preservado e nenhuma escrita em `accounts` ou `contacts`. **HubSpot ingest apply preflight (C2.9E.2B.2B.1) está fechado tecnicamente e validado funcionalmente**, com `canApply=false`, `wouldPersist=false` e nenhuma escrita canônica. **HubSpot ingest apply RPC foundation (C2.9E.2B.2B.2A) está fechado tecnicamente e validado manualmente no Supabase SQL Editor**, com RPC criada/atualizada, `Success. No rows returned`, `contacts.id` fora do patch mutável e nenhum apply real executado. **HubSpot ingest protected apply route (C2.9E.2B.2B.2B) está fechado tecnicamente e validado funcionalmente em modo bloqueado**, com feature flag obrigatória, `HTTP 403` sem flag e sem chamada à RPC. **HubSpot identity mapping foundation (C2.9E.2C.1) está fechado tecnicamente e validado manualmente no Supabase SQL Editor**, com a tabela `hubspot_identity_mappings` criada, `rls_enabled=true`, `policies_count=3`, `mappings_count=0` e nenhum apply real executado. **HubSpot identity mapping recovery dry-run (C2.9E.2C.2) está fechado tecnicamente e validado funcionalmente em leitura pura**, com `accounts` e `contacts` totalmente unresolved. **HubSpot identity mapping proposal dry-run (C2.9E.2C.3) está fechado tecnicamente e validado manualmente no Supabase SQL Editor**, com `348` accounts no snapshot, `0` propostas seguras, `0` ambíguas, `0` unsafe e `348` unresolved; a conclusão operacional é não persistir mappings agora e investigar o desalinhamento entre a base HubSpot e a base canônica atual antes de qualquer insert. **Decisão operacional HubSpot x Canopi (C2.9E.2D.7) registrada em `57-hubspot-operational-decision-clean-reload.md`**: HubSpot atual como histórico auditável, nova carga limpa a partir da Canopi atual, Canopi como fonte de verdade e contrato de identidade antes de qualquer nova carga. **HubSpot property registry formal (C2.9E.2D.9) implementado em `src/lib/hubspotPropertyRegistry.ts`**: 43 propriedades registradas cobrindo Company (11), Contact (13), Deal (10) e Product/Service (9); helpers `getHubspotPropertiesByEntity`, `getBlockingHubspotProperties`, `getRequiredHubspotProperties`, `isRegisteredHubspotProperty` e `validateHubspotPropertyRegistry` disponíveis; nenhuma propriedade real criada no HubSpot; doc operacional em `58-hubspot-property-registry.md`. **HubSpot clean reload dry-run (C2.9E.2D.10) implementado em `src/lib/server/hubspotCleanReloadDryRunService.ts`** e rota `POST /api/account-connectors/hubspot/clean-reload/dry-run`: lê `accounts` e `contacts` da Canopi somente em leitura, valida contra a property registry (blocking properties obrigatórias, ancoragem contact→company, tenantId), gera `planHash` determinístico SHA-256, retorna `canProceedToCleanCreate: true/false`; nenhuma escrita no HubSpot, Supabase ou mappings; doc operacional em `59-hubspot-clean-reload-dry-run.md`. **HubSpot clean reload dry-run validação funcional (C2.9E.2D.10A) concluída**: 6 testes negativos confirmados (BATCH_ID_REQUIRED, 5 chaves bloqueadas, tenantId ausente bloqueando todas as 250 companies), teste positivo com `canProceedToCleanCreate: true`, `companies.valid: 247`, `companies.blocked: 3` (legados sem nome), `contacts.valid: 305`, `existingMappings.total: 0`, `planHash` determinístico confirmado, nenhum token na resposta e nenhuma escrita executada; doc operacional em `60-hubspot-clean-reload-dry-run-functional-validation.md`. **HubSpot clean reload setup de propriedades bloqueadoras (C2.9E.2D.11) implementado** em `src/lib/server/hubspotCleanReloadSetupService.ts` e rota `POST /api/account-connectors/hubspot/clean-reload/setup-properties`: verifica e opcionalmente cria as 6 propriedades bloqueadoras (Company: `canopi_canonical_id`, `canopi_company_id`, `canopi_tenant_id`; Contact: `canopi_canonical_id`, `canopi_contact_id`, `canopi_tenant_id`) usando token de env; modo `verify` (somente leitura, padrão) e modo `create_missing` (requer `confirm=true`); propriedades incompatíveis bloqueiam `create_missing`; re-verificação automática pós-criação; nenhum Company, Contact ou mapping criado; validado funcionalmente (C2.9E.2D.11A): verify inicial 2 ok + 4 missing, create_missing criou 4 propriedades (`canopi_canonical_id` e `canopi_tenant_id` para Company e Contact), re-verify confirmou 6/6 ok e `canProceedToCleanCreate: true`; nenhum token retornado; doc operacional em `61-hubspot-clean-reload-setup-properties.md`. **HubSpot clean reload create limpo de Companies (C2.9E.2D.12) implementado** em `src/lib/server/hubspotCleanReloadCompanyCreateService.ts` e rota `POST /api/account-connectors/hubspot/clean-reload/create-companies`: preflight mandatório (registry, setup properties, existing mappings, valid accounts), create em chunks de 50 via `POST /crm/v3/objects/companies/batch/create`, captura de `hs_object_id`, matching por `canopi_canonical_id`, persistência de mappings em `hubspot_identity_mappings` (`entity_type=account`); `confirmCreateCompanies: true` obrigatório; chaves bloqueadas (`token`, `contacts`, `deals`, `products`, `services`, `reset`, `delete`, `upsert`, `applyAll`); nenhum Contact, Deal ou Product criado; nenhuma escrita em `accounts` ou `contacts` no Supabase; `canProceedToContactCreate: boolean` na resposta; validado funcionalmente (C2.9E.2D.12A): preflight completo, 247 Companies criadas no HubSpot, 247 mappings persistidos, `canProceedToContactCreate: true`; domain sanitization adicionada (domínios com `&` ou caracteres inválidos omitidos do payload); `resumeMode` adicionado para retomar após falha parcial; detecção de conflito `canopi_company_id` histórico adicionada ao preflight; retry 429 adicionado ao batch create; doc operacional em `62-hubspot-clean-reload-company-create.md`. **RD Station CRM continua postergado** até encerramento formal da frente HubSpot ou decisão explícita. **Salesforce C4.18C permanece fechado** e não foi reaberto. lint e tsc passaram.
...
### Atualização HubSpot C2.9E.2C.2
- Dry-run de recuperação de mappings de identidade fechado tecnicamente e validado funcionalmente em `ed53cc0`.
- Serviço confirmado em leitura pura: `src/lib/server/hubspotIdentityMappingRecoveryDryRunService.ts`.
- Contrato validado: `d673e0d5-7a9c-4c01-bd64-a74e6f2bda12`.
- `status=success`, `mode=identity_mapping_recovery_dry_run`, `snapshot.status=planned`.
- `accounts`: `348` total, `348` unresolved.
- `contacts`: `736` total, `736` unresolved.
- `matchSource` diferencia `direct_canonical_id`, `identity_mapping_canopi_external_id` e `identity_mapping_hubspot_id`.
- Próximo recorte operacional: recuperar/popular mappings de forma controlada e só depois regenerar snapshot, endurecer preflight e adaptar a RPC.

### Atualização HubSpot C2.9E.2C.3
- Proposta controlada de recuperação de mappings fechada tecnicamente e validada manualmente no Supabase SQL Editor em `2119893`.
- Serviço confirmado em leitura pura: `src/lib/server/hubspotIdentityMappingProposalDryRunService.ts`.
- Validação manual read-only executada com o contrato `d673e0d5-7a9c-4c01-bd64-a74e6f2bda12`.
- `total_accounts=348`, com `accounts_with_nome=348`, `accounts_with_dominio=348` e `accounts_with_nome_e_dominio=348`.
- Resultado funcional: `proposed_exact=0`, `proposed_strong=0`, `ambiguous=0`, `unsafe=0`, `unresolved=348`, `missing_required_fields=0`.
- Não houve match seguro por ID canônico, mapping, nome, domínio ou par nome+domínio.
- Contagens observadas após a validação: `hubspot_identity_mappings=0`, `hubspot_ingest_contracts=3`, `accounts=250`, `contacts=305`.
- Decisão operacional: não persistir mappings agora; investigar o desalinhamento entre a base HubSpot e a base canônica atual antes de qualquer insert.
- Próximo recorte operacional: só considerar persistência controlada depois de esclarecer a origem do desalinhamento.
