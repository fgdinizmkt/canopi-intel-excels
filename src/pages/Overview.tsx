...
import { TrendingUp, TrendingDown, AlertCircle, CheckCircle2, ShieldCheck, Target, Rocket, MoreHorizontal, Sparkles } from 'lucide-react';
import { kpis, accountsAtRisk, priorities, pipelineInfluence } from '../data/mockData';
      ...
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-8 rounded-3xl border border-slate-100 shadow-sm relative overflow-hidden">
        <div className="relative z-10">
          ...
          <p className="text-slate-500 mt-2 max-w-md font-medium">
            Sua central de inteligência está sincronizada. Detectamos <span className="text-brand font-bold">12 novos sinais</span> de alta intenção hoje.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          <Card className="border-l-4 border-l-brand relative overflow-hidden group">
            <div className="relative z-10">
              ...
              <h2 className="text-3xl font-bold text-slate-900 mb-4">CloudTech Solutions Inc.</h2>
              <p className="text-slate-600 max-w-xl leading-relaxed mb-8 text-sm">
                Detectamos engajamento simultâneo de 4 tomadores de decisão nos últimos 2 dias. 
                Atividade concentrada em "Expansão de Infraestrutura".
              </p>
          ...
          <div className="space-y-4">
            ...
            <section className="grid grid-cols-2 lg:grid-cols-4 gap-4">
              {[
                { label: 'SEO / ORGÂNICO', status: 'Alta', variant: 'emerald' },
                { label: 'INBOUND', status: 'Estável', variant: 'emerald' },
                { label: 'OUTBOUND', status: 'Alerta', variant: 'amber' },
                { label: 'TRÁFEGO PAGO', status: 'Baixa', variant: 'red' },
              ].map((channel, i) => (
          ...
          {/* New Chart: Influência de Pipeline por Origem */}
        ...
        <div className="space-y-8">
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-4">
              <div className="p-4 bg-white rounded-2xl border-l-4 border-red-500 shadow-sm">
                <p className="text-xs font-bold text-red-900">Campanhas perdendo performance</p>
                <p className="text-[10px] text-red-600 mt-1">CPC do Google Ads subiu 42% nas últimas 48h.</p>
              </div>
              <div className="p-4 bg-white rounded-2xl border-l-4 border-amber-500 shadow-sm">
                <p className="text-xs font-bold text-amber-900">Contas sem engajamento</p>
                <p className="text-[10px] text-amber-600 mt-1">12 contas Tier 1 não recebem contato há &gt; 7 dias.</p>
              </div>
              <div className="p-4 bg-white rounded-2xl border-l-4 border-brand shadow-sm">
                <p className="text-xs font-bold text-brand">Hot leads sem follow-up</p>
                <p className="text-[10px] text-brand mt-1">8 leads com intenção alta aguardando resposta.</p>
              </div>
            </div>
          </div>
          ...
          <Card title="Prioridades Imediatas">
            <div className="space-y-5">
              {priorities.map((priority) => (
                <div key={priority.id} className="flex items-center justify-between group cursor-pointer">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-xl bg-slate-100 flex items-center justify-center text-xs font-bold text-slate-500 uppercase">
                      {priority.account.substring(0, 2)}
                    </div>
                    <div>
                      <p className="text-sm font-bold text-slate-900 group-hover:text-brand transition-colors">{priority.title}</p>
                      <p className="text-[10px] text-slate-400 font-medium uppercase tracking-wider">{priority.account} · {priority.owner}</p>
                    </div>
                  </div>
                  <Badge variant={priority.severity === 'crítica' ? 'red' : priority.severity === 'média' ? 'amber' : 'slate'}>
                    {priority.severity}
                  </Badge>
                </div>
              ))}
          ...
          <Card title="Prontidão ABM/ABX" subtitle="Contas com picos de intenção">
            <div className="space-y-4">
              {[
                { name: 'Makaroff Enterprise', info: '6 novos visitantes hoje', score: 88, status: 'Alta Intenção' },
                { name: 'Viper Pharma', info: 'Demo visualizada 3x', score: 92, status: 'Pico de Sinal' },
              ].map((item, i) => (
                <div key={i} className="flex items-center justify-between p-4 rounded-2xl border border-slate-50 hover:bg-slate-50 transition-colors cursor-pointer">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] text-slate-500 font-medium">{item.info}</p>
                    </div>
                  </div>
                  <div className="text-right">
                    <p className="text-[10px] font-bold text-emerald-600 uppercase tracking-widest">{item.status}</p>
                    <p className="text-[10px] text-slate-400 font-medium">Score: {item.score}</p>
                  </div>
                </div>
              ))}
